<script>
	...
	import { visitor } from '../stores/user.js';
...
<div class="w-full py-6 flex justify-center text-white">
	<div
		class="w-5/6 px-8 md:w-4/6 lg:w-1/2 max-w-96 py-12 text-center"
	>
        {#if !verified}
        <div class="items-center cursor-pointer" >
			<h2 class="cursor-pointer h-full w-full flex p-2 pb-10 justify-center items-center text-2xl">
			...
			<form class="space-y-4 md:space-y-8">
				...
				{:else if sendAgain === false}
					<div transition:slide class="pt-6">
						<div
							on:click|preventDefault={verifyEmail}
						>
			...
			<div class="pt-6 space-y-6">
				<h2
					class="text-mm-600 hover:text-mm-800 cursor-pointer"
					on:click={toggleSendAgain}
				>
				...
				{#if sendAgain}
					<div transition:slide class="space-y-6">
						...
						{:else}
							<div
								on:click|preventDefault={sendVerificationLink}
							>
								<Button icon="mail"
									>Send me the code again</Button
								>
							</div>
						{/if}

						<p class="opacity-60">
							Don't forget to check in your spam folder as well
						</p>
					</div>
				{/if}
			</div>
		...
		{:else if verified}
			...
			>
				...
				<h2
					class="pt-12 pb-6 text-mm-600 font-bold text-2xl uppercase"
				>
					verified
				</h2>
				<p class="text-mm-800">
